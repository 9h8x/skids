...
def consulta_dni(dni):
    ...
    payload_login = {'username': 'AGILCRED SA.', 'password': 'GERENCIA'}
    session = requests.Session()
    response_login = session.post(login_url, data=payload_login)
    if response_login.status_code == 200:
        print('Inicio de sesión exitoso')
        payload_consulta = {'id': '39414730', 'dni': dni, 'cuit': None, 'sexo': None}
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36', 'Accept': 'application/json, text/javascript, /; q=0.01', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'Origin': 'https://informescea.com.ar/', 'Referer': 'https://informescea.com.ar/index.php', 'X-Requested-With': 'XMLHttpRequest'}
