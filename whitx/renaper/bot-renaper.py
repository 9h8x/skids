...
@bot.command()
async def help(ctx):
    embed = discord.Embed(title='📘 Comandos disponibles', description='Lista de comandos del bot RENAPER:', color=45300)
    ...
    embed.add_field(name='.dni [DNI]', value='🔍 Muestra información personal básica del ciudadano.', inline=False)
    embed.add_field(name='.banco [DNI]', value='💰 Muestra información financiera del ciudadano, como deudas, cheques y afectaciones.', inline=False)
    embed.add_field(name='.propiedades [DNI]', value='🏠 Muestra las propiedades registradas del ciudadano.', inline=False)
    embed.add_field(name='.infoempleos [DNI]', value='🧾 Muestra la información laboral y empleadores del ciudadano.', inline=False)
